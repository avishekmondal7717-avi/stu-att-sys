...
const Register = () => {
  ...
  const [createdStudentPayload, setCreatedStudentPayload] = useState(null);
  ...
  const simulateEmailVerification = async () => {
    try {
      const updatedPayload = { ...createdStudentPayload, status: "Active" };
      await studentAPI.update(createdStudentId, updatedPayload);
      localStorage.removeItem(`pending_verification_${formData.email}`);
      setSuccessMessage("Email verified successfully! You can now log in.");
    } catch (err) {
      console.error(err);
      setEmailError("Simulation verification failed.");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (emailErr || passwordErr || phoneErr || confirmPasswordErr || dobErr || studentIdErr) {
      ...
      setDobError(dobErr);
      setStudentIdError(studentIdErr);
      setSuccessMessage("");
      return;
    }
          ...
          <form onSubmit={handleSubmit} className="register-form">
            ...
            <div className="register-row">
              ...
              <div className="register-field">
                ...
                <div className={`register-input-wrap ${studentIdError ? "input-error" : ""}`}>
                  ...
                  <input type="text" name="studentId" placeholder="e.g. STU2026001" value={formData.studentId} onChange={handleChange} required />
                </div>
                {studentIdError && <p className="error-msg">{studentIdError}</p>}
              </div>
            </div>
